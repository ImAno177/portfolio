...
const ACTIONS: readonly CatAction[] = [
  "idle",
  "walk",
  "sit",
  "sleep",
...
function needInfluence(cat: CatAgent, action: CatAction): number {
  if (action === "sleep")
    return (1 - cat.needs.energy) * 0.9 + cat.personality.sleepiness * 0.65;
  if (action === "eat")
    return cat.needs.hunger * 0.85 + cat.personality.foodMotivation * 0.65;
  if (action === "play")
    return (1 - cat.needs.fun) * 0.7 + cat.personality.playfulness * 0.75;
  if (action === "meow" || action === "react")
    return (1 - cat.needs.social) * 0.35 + cat.personality.socialness * 0.24;
  if (action === "observe") return cat.personality.curiosity * 0.7;
  if (action === "groom")
    return cat.needs.comfort * 0.2 + (1 - cat.needs.comfort) * 0.45;
  return 0;
}

function personalityInfluence(cat: CatAgent, action: CatAction): number {
  if (action === "walk")
    return cat.personality.curiosity * 0.32 + cat.personality.chaos * 0.16;
...
export function scoreActions(
  ...
  const randomJitter = clamp(context.randomJitter);
  return ACTIONS.map((action) => {
    const base =
      action === "react" || action === "meow"
        ? profile.social
        : (profile[action as keyof TimeProfile] ?? 0.1);
    ...
    const occupiedPenalty =
      action === "walk" &&
      context.occupiedZones.has(cat.targetZone ?? cat.currentZone)
        ? 0.14
        : 0;
    const recentPenalty = context.recentActions.includes(action) ? 0.2 : 0;
    const switchingPenalty =
      context.elapsedInState < MIN_STATE_DURATION_MS &&
      action !== cat.currentState
        ? 0.7
        : 0;
    const cooldownPenalty =
      (cat.actionCooldowns[action] ?? 0) > context.now ? 100 : 0;
    const jitter = randomJitter * (0.08 + cat.personality.chaos * 0.16);
    return {
      action,
      score: Math.max(
        0,
...
export function advanceNeeds(
  ...
  const boundedMinutes = Number.isFinite(minutes)
    ? Math.min(24 * 60, Math.max(0, minutes))
    : 0;
...
type NeedRates = Partial<Record<keyof CatNeeds, number>>;

const ACTION_NEED_RATES: Partial<Record<CatAction, NeedRates>> = {
  idle: {
    energy: -0.008,
    hunger: 0.012,
    social: -0.006,
  ...
  eat: { energy: 0.06, hunger: -1.2, comfort: 0.02 },
  play: { energy: -0.3, hunger: 0.04, social: 0.04, fun: 0.6 },
  groom: { energy: -0.01, hunger: 0.006, fun: -0.002, comfort: 0.5 },
};

export function advanceActionNeeds(
  needs: CatNeeds,
  seconds: number,
  action: CatAction,
): CatNeeds {
  const boundedSeconds = Number.isFinite(seconds)
    ? Math.min(24 * 60 * 60, Math.max(0, seconds))
    : 0;
  // Active actions produce a visible result during their short clips; idle decay is hourly.
  const scale =
    boundedSeconds /
    (action === "sleep" ||
    action === "eat" ||
    ...
    action === "groom"
      ? 60
      : 3600);
  const rates = ACTION_NEED_RATES[action] ?? ACTION_NEED_RATES.idle ?? {};
  return {
    energy: clamp(needs.energy + (rates.energy ?? 0) * scale),
    hunger: clamp(needs.hunger + (rates.hunger ?? 0) * scale),
    social: clamp(needs.social + (rates.social ?? 0) * scale),
    fun: clamp(needs.fun + (rates.fun ?? 0) * scale),
    comfort: clamp(needs.comfort + (rates.comfort ?? 0) * scale),
  };
}
